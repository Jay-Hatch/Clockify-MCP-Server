...
- `read_audit_log_report` requires Clockify's audit log feature, which is an Enterprise workspace feature. The audit author filter uses `authorIds`, and `SYSTEM` can be included for system events. Audit log reports are limited to a maximum 30-day date range.
